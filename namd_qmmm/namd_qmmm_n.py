...
class QMMM(object):
    def __init__(self,
        ...
        if self.elecMode.lower() in {'truncation', 'mmewald'}:
            ...
            raise ValueError(
                "Only 'truncation', 'mmewald', and 'qmewald' are supported at the moment."
            )
        ...
        if self.qmChargeMode == 'esp':
            ...
            raise ValueError(
                "Please choose 'mulliken','esp','chelpg', 'none', and 'zero' for qmChargeMode."
            )
        ...
        if self.qmSwitching.lower() == 'on':
            if self.elecMode.lower() == 'qmewald':
                raise ValueError(
                    "Can not use elecMode='qmewald' with qmSwitching='on'.")
        ...
        if self.qmElecEmbed.lower() == 'on':
            ...
            if self.elecMode.lower() == 'qmewald':
                raise ValueError(
                    "Can not use elecMode='qmewald' with qmElecEmbed='off'.")
    ...
    def get_namdinput(self):
        ...
        self.cmd = os.popen(
            "ps -p %s -ocommand=" % self.pid).read().strip().split()
        ...
        if hasattr(self.QM, 'qmEnergy'):
            ...
            with open(self.QM.fin + ".result", 'w') as f:
                ...
                for i in range(self.QM.numQMAtoms):
                    f.write(" ".join(
                        format(j, "22.14e") for j in self.QM.qmForces[i]
                    ) + "  " + format(self.QM.qmChrgs4MM[i], "22.14e") + "\n")
                for i in range(self.QM.numRPntChrgs):
                    f.write(" ".join(
                        format(j, "22.14e")
                        for j in self.QM.pntChrgForces[i]) + "\n")
        else:
    ...
    def save_extforces(self):
        ...
        with open(self.QM.baseDir + "extforce.dat", 'w') as f:
            for i in range(self.QM.numAtoms):
                f.write("%4d    0  " % (i + 1) + "  ".join(
                    format(j, "22.14e") for j in self.mmForces[i]) + "\n")
            f.write("0.0")
    ...
    def save_pntchrgs(self):
        ...
        if hasattr(self.QM, 'pntScale'):
            mmScale[self.QM.pntIdx[0:self.QM.numRPntChrgs]] = self.QM.pntScale[
                0:self.QM.numRPntChrgs]
            mmDist[self.QM.pntIdx[0:self.QM.numRPntChrgs]] = self.QM.pntDist
        else:
            mmScale[self.QM.pntIdx[0:self.QM.numRPntChrgs]] += 1
        mmChrgs[self.QM.pntIdx[0:self.QM.numRPntChrgs]] = self.QM.pntChrgs4QM[
            0:self.QM.numRPntChrgs]

        mmScale[self.QM.qmIdx[0:self.QM.numRealQMAtoms]] = np.ones(
            self.QM.numRealQMAtoms)
        mmChrgs[self.QM.qmIdx[0:self.QM.numRealQMAtoms]] = self.QM.qmChrgs4MM[
            0:self.QM.numRealQMAtoms]
